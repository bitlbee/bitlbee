...
struct groupchat *msn_sb_to_chat(struct msn_switchboard *sb)
{
	...
	if (sb->who) {
		imcb_chat_add_buddy(sb->chat, sb->who);
	}
...
void msn_sb_destroy(struct msn_switchboard *sb)
{
	...
	if (sb->handler) {
		if (sb->handler->rxq) {
			g_free(sb->handler->rxq);
		}
		if (sb->handler->cmd_text) {
			g_free(sb->handler->cmd_text);
		}
		g_free(sb->handler);
	}

	if (sb->inp) {
		b_event_remove(sb->inp);
	}
	closesocket(sb->fd);
...
static int msn_sb_command(struct msn_handler_data *handler, char **cmd, int num_parts)
{
	...
	if (strcmp(cmd[0], "XFR") == 0) {
		imcb_error(ic,
		           "Received an XFR from a switchboard server, unable to comply! This is likely to be a bug, please report it!");
		imc_logout(ic, TRUE);
		return(0);
		...
		if (sb->who) {
			...
			imcb_log(ic,
			         "The MSN servers could not deliver one of your groupchat messages to all participants.");
		}
		...
		if (sb->who) {
			...
			debug("Person %s left the one-to-one switchboard connection. Keeping it around as a spare...",
			      cmd[1]);
